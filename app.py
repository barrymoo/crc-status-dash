...
def generate_table():
    return html.Table(
        # Header
        [html.Tr([
            html.Th(x) for x in ["Cluster", "Used Cores", "Total Cores"]
        ])] +
        # Body
        generate_html_tr('smp') + generate_html_tr('gpu') +
        generate_html_tr('mpi') + generate_html_tr('htc')
    )
...
def generate_figure():
    ...
    concat = pd.concat([smp.set_index('time'), gpu.set_index('time'),
                        mpi.set_index('time'), htc.set_index('time')],
                       axis='columns', keys=['smp', 'gpu', 'mpi', 'htc'])
    ...
    traces = [go.Scatter(
                ...
                name=cluster,
                mode='lines+markers') for cluster in ['smp', 'gpu', 'mpi', 'htc']]
    layout = go.Layout(
